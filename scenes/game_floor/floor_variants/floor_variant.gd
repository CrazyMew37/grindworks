...
func get_calculated_level_range(_difficulty: int) -> Vector2i:
	var base_range := Vector2i(ceili(((2) + (floori((Util.floor_number) / 10.0) / 2.0)) * (Util.floor_number + 1.0)), ceili(((3) + (floori((Util.floor_number) / 10.0) / 2.0)) * (Util.floor_number + 1.0)))
	return base_range
