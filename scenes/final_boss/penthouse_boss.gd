...
func _ready() -> void:
	...
	boss_cog.stats.max_hp = ceili(10.0 * (boss_cog.level * boss_cog.level) * (ceili(Util.floor_number / 5) * 0.75))
	boss_cog.stats.hp = ceili(10.0 * (boss_cog.level * boss_cog.level) * (ceili(Util.floor_number / 5) * 0.75))

	boss_cog_2.health_mod = 1.0
	boss_cog_2.stats.max_hp = ceili(10.0 * (boss_cog.level * boss_cog.level) * (ceili(Util.floor_number / 5.0) * 0.75))
	boss_cog_2.stats.hp = ceili(10.0 * (boss_cog.level * boss_cog.level) * (ceili(Util.floor_number / 5.0) * 0.75))
