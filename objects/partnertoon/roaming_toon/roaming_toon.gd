...
func get_attack() -> ToonAttack:
	if not doodle_actions.is_empty():
		...
		var playerdamagestat = BattleService.ongoing_battle.battle_stats[Util.get_player()].damage
		action.damage = round(((action.damage + (Util.floor_number + 1)) * (Util.floor_number + 1)) * playerdamagestat * 0.8) / cogstat.get_stat('defense')
...
func teleport_away(new_state := DoodleState.STOPPED) -> void:
	...
	tween = create_tween()
	tween.tween_callback(func(): toon.right_hand_bone.add_child(hole))
	tween.tween_callback(func(): hole.position = Vector3(-0.2,1.0,0.25))
	tween.tween_callback(func(): hole.rotation_degrees.z = 90.0)
	tween.tween_callback(set_animation.bind('teleport'))
	tween.tween_callback(func(): AudioManager.play_sound(load("res://audio/sfx/toon/AV_teleport.ogg")))
	tween.tween_interval(1.6)
	tween.tween_callback(hole.reparent.bind(toon.get_node('DropShadow')))
	tween.parallel().tween_callback(func(): hole.scale = Vector3(0.8,0.8,0.8))
	tween.parallel().tween_callback(func(): hole.position = Vector3(0.0,0.0,1.5))
	tween.parallel().tween_callback(func(): hole.rotation = Vector3(0,0,0))
	tween.tween_interval(1.3333)
	tween.tween_callback(func(): toon.get_node('DropShadow').hide())
	tween.tween_callback(func(): hole.get_node('AnimationPlayer').play('shrink'))
	tween.tween_interval(0.5)
	tween.tween_callback(hole.queue_free)
	tween.tween_callback(toon.hide)
...
func teleport_in(new_state := DoodleState.STOPPED) -> void:
	...
	tween = create_tween()
	tween.tween_callback(func(): toon.legs.position.y -= 10.0)
	var hole : Node3D = load('res://objects/misc/teleport_hole/teleport_hole.tscn').instantiate()
	tween.tween_callback(add_child.bind(hole))
	tween.tween_callback(func(): hole.position.y = 0.01)
	tween.tween_callback(func(): hole.scale *= 0.4)
	tween.tween_callback(func(): hole.get_node('AnimationPlayer').play('grow'))
	tween.tween_interval(0.5)
	tween.tween_callback(toon.show)
	tween.tween_callback(func(): toon.get_node('DropShadow').show())
...
func cancel_anim() -> void:
	if tween: tween.custom_step(10000.0)
