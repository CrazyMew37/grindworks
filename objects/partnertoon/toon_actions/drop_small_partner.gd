...
func action():
	...
	if hit:
		...
		if not get_immunity(target):
			...
			apply_debuff_partner(target, damage_dealt)
...
func apply_debuff_partner(target: Cog, damage_dealt: int) -> void:
	...
	if Util.get_player().stats.get_stat("drop_aftershock_round_boost") != 0:
		new_effect.rounds += Util.get_player().stats.get_stat("drop_aftershock_round_boost")
