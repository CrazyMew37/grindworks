...
func action():
	var playerdude = Util.get_player()
	...
	if hit:
		...
		if not immune:
			...
			if playerdude.throw_heals:
				playerdude.quick_heal(roundi(throw_damage * playerdude.stats.get_stat("throw_heal_boost")))
		else:
