...
class_name StatBoostCritMult

var ICONS := {
	'crit_mult': load("res://ui_assets/battle/statuses/critmultiplier.png")
}
...
func combine(effect: StatusEffect) -> bool:
	if not effect is StatBoostCritMult:
		return false
	
	if force_no_combine or effect.force_no_combine:
		return false

	if effect is StatBoostCritMult:
