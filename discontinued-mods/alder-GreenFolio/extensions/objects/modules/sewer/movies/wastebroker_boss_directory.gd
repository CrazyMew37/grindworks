...
@onready var fourth_cam: Camera3D = %FourthCam
@onready var cog: Cog = %WasteBroker
...
func on_round_start(_actions: Array[BattleAction], _manager: BattleManager) -> void:
	...
	if manager.current_round % 2 == 1:
		attack = load("res://mods-unpacked/alder-GreenFolio/extensions/objects/battle/battle_resources/misc_movies/wb_junkbond.tres").duplicate(true)
		attack.targets = [self]
		attack.user = cog
